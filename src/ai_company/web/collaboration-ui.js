...
export function createCollaborationUI({esc,badge,stamp,readable,evidenceLinks,label,documents}) {
  ...
  let observer;
  let activeProject='';
  const kinds={pm:'PM',role:'프로젝트 역할',check:'검사',reviewer:'독립 검수',final:'최종 검수',translator:'한국어 번역',document:'문서'};
  ...
  function nodeCard(node,overview,selected,linked){
    ...
    const taskId=node.current_task_id,observed=a.observed||{},requested=a.requested||{};
    const recent=(overview.collaboration?.transfers||[]).find(t=>t.from===node.id||t.to===node.id);
    const model=observed.status==='observed'&&observed.model?`관측 모델 · ${observed.model}`:requested.model?`요청 모델 · ${requested.model}`:'요청 모델 미설정';
    const effort=observed.status==='observed'&&observed.reasoning_effort?`관측 추론 · ${observed.reasoning_effort}`:requested.reasoning_effort?`요청 추론 · ${requested.reasoning_effort} / 적용 미확인`:'추론 설정 미확인';
    return `<article class="${node.kind==='role'?'role ':''}collab-node ${selected?'is-selected':''} ${linked?'is-linked':''}" data-node-id="${esc(node.id)}"><button type="button" class="collab-node-select" id="node-${esc(node.id)}" data-collaboration-node="${esc(node.id)}" aria-pressed="${selected}"><span class="collab-kind">${esc(kinds[node.kind]||node.kind)}</span><span class="collab-node-title">${esc(node.name)}</span>${badge(node.status)}</button><p class="collab-responsibility">${esc(node.responsibility||'책임 범위 미정')}</p><div class="collab-node-body"><div class="task-row"><div class="eyebrow">현재 작업</div><p>${esc(task?.title||taskId||'배정된 작업 없음')}</p></div>${node.wait_reason||node.resume_at?`<div class="role-note">${esc(documents.text(`task:${taskId}:reason`,'reason',node.wait_reason||label(node.status)))}${node.resume_at?`<br>재확인 예약 · ${stamp(node.resume_at)}`:''}</div>`:''}${node.kind==='document'?'<p class="small muted">시스템 원문 · 모델 역할 아님</p>':`<p class="collab-model"><strong>${esc(model)}</strong><span>${esc(effort)}</span></p>`}${recent?`<button type="button" class="collab-recent" data-collaboration-transfer="${esc(recent.id)}"><span>최근 전달 · ${esc(statuses[recent.status]||recent.status)}</span><strong>${esc(recent.title)}</strong><time>${stamp(recent.created_at)}</time></button>`:''}${node.handoffs?.length?`<p class="small muted">역할 유지 · 담당 이관 ${node.handoffs.length}건</p>`:''}</div></article>`;
  }
  function detail(data,overview,selection){
    if(!selection)return '<div class="collaboration-detail-empty"><h3>상세</h3><p>역할 또는 전달 행을 선택하면 담당 설정·작업·근거를 확인할 수 있습니다.</p></div>';
