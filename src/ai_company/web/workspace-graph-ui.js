...
export function createWorkspaceGraph({esc,embedded=false,diagramLinks=false,label=v=>statusLabels[v]||v,stamp=v=>v?new Date(v*1000).toLocaleString('ko-KR'):'미기록',onInteractionEnd=()=>{},onScopeChange=()=>{}}){
 ...
 function body(project){const record=projects.get(project),snapshot=scope(project);if(!record||!snapshot){
  if(record?.missingSelection)return `<header class="rg-header">${embedded?'':'<h2>진행</h2>'+snapshotSelector(record,null)}</header><p class="rg-alert" role="status" data-rg-missing-scope>선택한 실행 또는 계획이 이번 조회에 없습니다. 다른 실행으로 대신 표시하지 않습니다. 대상을 다시 선택해 주세요.</p>`;
  ...
  return `<header class="rg-header"><div>${embedded?'':'<h2>진행</h2>'}<p>${snapshot.source==='fixture'?'예시 기록 · 실제 실행 아님':snapshot.mode==='planned'||!snapshot.run_id?'계획 · 실제 실행 전':'저장된 실행 기록'}</p></div>${embedded?'':snapshotSelector(record,snapshot)}</header>${(snapshot.warnings||[]).map(w=>`<p class="rg-alert">${esc(w)}</p>`).join('')}${record.disconnected?'<p class="rg-alert" role="status">연결이 끊겼습니다. 마지막 조회 자료를 표시합니다.</p>':''}<div class="rg-tools">${diagramLinks&&snapshot.plan_id?`<a class="rg-export" href="/diagram-view.html?project=${encodeURIComponent(project)}&snapshot=${encodeURIComponent(snapshot.id)}&fingerprint=${encodeURIComponent(snapshot.fingerprint)}" target="_blank" rel="noopener">그림</a>`:''}<div role="group" aria-label="진행 보기">${button('그래프','graph',`aria-pressed="${v.mode==='graph'}"`)}${button('목록','list',`aria-pressed="${v.mode==='list'}"`)}</div>${v.mode==='graph'?`<div role="group" aria-label="그래프 탐색">${button('−','zoom-out','aria-label="축소"')}${button('+','zoom-in','aria-label="확대"')}${button('전체 보기','fit')}${button('이동','pan',`aria-pressed="${v.panMode}"`)}</div><output class="rg-zoom" aria-label="확대 비율">${Math.round(v.zoom*100)}%</output>`:''}</div><div class="rg-layout"><section class="rg-map" aria-label="관계 보기">${snapshot.nodes.length?v.mode==='graph'?graph(snapshot,v):list(snapshot,v):`<p class="rg-empty">${esc(snapshot.empty_reason||'저장된 역할이 없습니다. PM의 역할 제안을 확인하세요.')}</p>`}${v.mode==='graph'&&[...(v.routes?.values()||[])].some(r=>r.issue||!r.label)?'<p class="rg-alert">배치가 좁아 일부 선이나 이름표가 겹칠 수 있습니다. 노드를 벌리거나 목록에서 관계를 선택하세요.</p>':''}<p class="rg-small">실선: 업무 흐름 · 짧은 점선: 이관 · 긴 점선: 되돌아가는 관계<br>예정 관계는 ‘예정’으로 표시합니다.<br>표시 ${snapshot.edges.length}건${Number.isInteger(history.total)?` / 조회 대상 ${history.total}건`:''}${history.complete===false?' · 과거 이력 일부만 포함':''}. 위치는 실행 순서를 뜻하지 않습니다.</p><details class="rg-help" ${helpOpen?'open':''}><summary id="rg-help-toggle">사용법</summary><p>노드·선을 눌러 상세를 엽니다. 이동 모드를 켜면 화면과 노드를 끌 수 있습니다. 키보드는 방향키로 화면 이동, 노드에서 Alt+방향키로 배치 이동합니다. 목록에서도 같은 기록을 선택할 수 있습니다.</p></details><p class="rg-update" role="status">${newCount?`새 전달 ${newCount}건`:'새 전달 없음'}</p></section>${detail(snapshot,v)}</div>`;
 }
 ...
 function mount(container,overview,project){
  ...
  function restoreFocus(id,kind){const attr=kind==='edge'?'data-rg-edge':'data-rg-node';const target=[...root.querySelectorAll(`[${attr}]`)].find(el=>el.getAttribute(attr)===id);(target||root.querySelector('.rg-viewport')||root.querySelector('[data-rg-snapshot]')||(embedded?container.querySelector('#journey-run'):null))?.focus({preventScroll:true});}
