...
export function createWorkspaceGraph({esc,label=v=>statusLabels[v]||v,stamp=v=>v?new Date(v*1000).toLocaleString('ko-KR'):'미기록'}){
 const projects=new Map(),views=new Map();let activeProject='',resizeObserver,disposeMount=()=>{};
 ...
 function ingest(overview,project){
  ...
  const prior=projects.get(project);if(prior&&(envelope.cursor<prior.data.cursor||envelope.cursor===prior.data.cursor&&envelope.observed_at<prior.data.observed_at))return false;
 ...
 function view(snapshot){const k=graphKey(snapshot);if(!views.has(k))views.set(k,{mode:'graph',selection:null,zoom:1,pan:{x:0,y:0},positions:{},small:null,panMode:false,seen:new Set(),focus:null});return views.get(k);}
 ...
 function nodeInfo(node){const a=node.assignment||{},r=a.requested||{},o=a.observed||{};return `<dl class="rg-facts"><div><dt>종류</dt><dd>${esc(kindLabels[node.kind]||node.kind)} · ${planned(node)?'예정':'저장된 기록'}</dd></div><div><dt>현재 일</dt><dd>${esc(node.current_task_title||node.task_title||node.current_work||node.responsibility||'연결된 작업 없음')}</dd></div><div><dt>요청</dt><dd>${esc(r.model||'미배정')} · ${esc(r.reasoning_effort||'추론 미설정')}</dd></div><div><dt>실제 적용</dt><dd>${o.status==='observed'?`${esc(o.model||'모델 미확인')} · ${esc(o.reasoning_effort||'추론 미확인')}`:'미확인'}</dd></div><div><dt>관측 근거</dt><dd>${esc(o.source||'없음')} · ${esc(o.scope||'미확인')}<br>모델의 자기 설명을 근거로 사용하지 않습니다.</dd></div>${node.wait_reason?`<div><dt>대기 이유</dt><dd>${esc(node.wait_reason)}${node.resume_at?`<br>재개 예약 ${esc(stamp(node.resume_at))}`:''}</dd></div>`:''}${a.quota?.status?`<div><dt>공유 한도</dt><dd>${esc(status(a.quota.status))}${a.quota.reset_at?` · ${esc(stamp(a.quota.reset_at))}`:''}</dd></div>`:''}</dl>${node.handoffs?.length?`<details><summary>담당 이관 ${node.handoffs.length}건</summary><p>역할과 작업은 유지하고 담당 세션만 바뀝니다.</p><pre>${esc(JSON.stringify(node.handoffs,null,2))}</pre></details>`:''}<details><summary>원본 참조</summary><pre>${esc(JSON.stringify(node.reference||{},null,2))}</pre></details>`;}
 ...
 function detail(snapshot,v){const selected=v.selection,item=selected&&(selected.kind==='node'?snapshot.nodes:snapshot.edges).find(n=>n.id===selected.id);return `<aside class="rg-detail ${item?'is-open':''}" aria-label="그래프 상세"><div class="rg-detail-top"><h3>${item?esc(selected.kind==='node'?item.name:relationLabels[item.kind]||item.kind):'상세'}</h3>${item?button('닫기','clear','aria-label="상세 닫기"'):''}</div>${item?`${selected.kind==='node'?`<p class="rg-state">${esc(status(item.status))}</p>${nodeInfo(item)}`:edgeInfo(item,snapshot)}`:selected?'<p>선택한 기록은 이번 응답에 없습니다. 과거 기록이 없다는 뜻은 아닙니다.</p>':'<p>노드나 전달선을 누르면 담당 모델과 산출물·대기 이유를 읽을 수 있습니다.</p>'}${refs(snapshot)}<div class="rg-reference-links">${(snapshot.approval_refs||[]).length?`<a href="#approvals?project=${encodeURIComponent(snapshot.project_id)}&run=${encodeURIComponent(snapshot.run_id)}">이 실행의 승인 ${(snapshot.approval_refs||[]).length}건</a>`:''}${(snapshot.report_refs||[]).length?`<a href="#reports?project=${encodeURIComponent(snapshot.project_id)}&run=${encodeURIComponent(snapshot.run_id)}">이 실행의 보고 ${(snapshot.report_refs||[]).length}건</a>`:''}</div><p class="rg-small">그래프 선택·위치 이동은 계획·모델·실행 권한을 변경하지 않습니다.</p></aside>`;}
 function summaryNode(node){const a=node.assignment||{},o=a.observed||{},r=a.requested||{};return `${o.status==='observed'?'관측':'요청'} · ${shortModel(o.status==='observed'?o.model:r.model)}`;}
 function graph(snapshot,v){const small=innerWidth<=700,layout=graphLayout(snapshot.nodes,small);
  if(v.small!==small){v.positions={};v.pan={x:0,y:0};v.zoom=1;v.small=small;}
  ...
  const paths=snapshot.edges.filter(e=>positions[e.from]&&positions[e.to]).map((e,index)=>{
   const from=positions[e.from],to=positions[e.to],reverse=e.kind==='revision_return'||to.y<=from.y;
   const x1=from.x+layout.nodeWidth/2,y1=from.y+(reverse?0:layout.nodeHeight),x2=to.x+layout.nodeWidth/2,y2=to.y+(reverse?layout.nodeHeight:0);
   const path=reverse?`M ${x1} ${y1} C ${Math.max(x1,x2)+90} ${y1-60}, ${Math.max(x1,x2)+90} ${y2+60}, ${x2} ${y2}`:`M ${x1} ${y1} C ${x1} ${(y1+y2)/2}, ${x2} ${(y1+y2)/2}, ${x2} ${y2}`;
   const selectedEdge=edge?.id===e.id||selected?.kind==='node'&&(e.from===selected.id||e.to===selected.id);
   return `<g class="rg-edge ${planned(e)?'is-planned':''} ${selectedEdge?'is-selected':''}" data-rg-edge-id="${esc(e.id)}"><path class="rg-edge-line" d="${path}" marker-end="url(#rg-arrow)"/><path class="rg-edge-hit" d="${path}" id="rg-edge-${esc(e.id)}" data-rg-edge="${esc(e.id)}" tabindex="0" role="button" aria-label="${esc(`${snapshot.nodes.find(n=>n.id===e.from)?.name}에서 ${snapshot.nodes.find(n=>n.id===e.to)?.name} · ${relationLabels[e.kind]||e.kind} · ${planned(e)?'예정':'기록'}`)}"/><text x="${(x1+x2)/2+9}" y="${(y1+y2)/2-8+index%2*16}" aria-hidden="true">${esc(relationLabels[e.kind]||e.kind)}</text></g>`;
  }).join('');
  return `<div class="rg-viewport ${v.panMode?'is-moving':''}" tabindex="0" role="group" aria-label="역할 그래프 · 방향키로 이동"><div class="rg-scene" style="width:${width}px;height:${height}px;transform:translate(${v.pan.x}px,${v.pan.y}px) scale(${v.zoom})"><svg class="rg-lines" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" aria-label="역할 관계"><defs><marker id="rg-arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z"/></marker></defs>${paths}</svg>${snapshot.nodes.map(node=>{const p=positions[node.id];return `<button type="button" class="rg-node ${selected?.kind==='node'&&selected.id===node.id?'is-selected':''} ${related.has(node.id)?'is-related':''} ${planned(node)?'is-planned':''}" style="left:${p.x}px;top:${p.y}px;width:${layout.nodeWidth}px;height:${layout.nodeHeight}px" id="rg-node-${esc(node.id)}" data-rg-node="${esc(node.id)}" aria-pressed="${selected?.kind==='node'&&selected.id===node.id}"><span class="rg-node-kind">${esc(kindLabels[node.kind]||node.kind)}${planned(node)?' · 예정':''}</span><strong>${esc(node.name)}</strong><span class="rg-node-state">${esc(status(node.status))}</span><span class="rg-node-model">${esc(summaryNode(node))}</span></button>`;}).join('')}</div></div>`;
 }
 ...
 function mount(container,overview,project){
  ...
  const controller=new AbortController(),signal=controller.signal;disposeMount=()=>controller.abort();
  ...
  function redraw(focus){root.innerHTML=body(project);if(focus)restoreFocus(focus.id,focus.kind);}
  ...
  root.addEventListener('change',event=>{if(event.target.matches('[data-rg-snapshot]')){projects.get(project).selected=event.target.value;redraw();root.querySelector('[data-rg-snapshot]')?.focus();}},{signal});
  root.addEventListener('click',event=>{if(dragged){dragged=false;return;}const n=event.target.closest('[data-rg-node]'),e=event.target.closest('[data-rg-edge]'),action=event.target.closest('[data-rg-action]')?.dataset.rgAction,s=scope(project);if(!s)return;const v=view(s);
   if(n||e){v.selection={kind:n?'node':'edge',id:n?n.dataset.rgNode:e.dataset.rgEdge};redraw(v.selection);return;}
   if(!action)return;
   if(['graph','list'].includes(action)){v.mode=action;redraw();root.querySelector(`[data-rg-action=${action}]`)?.focus();return;}
   if(action==='clear'){v.selection=null;redraw();root.querySelector('[data-rg-action=graph]')?.focus();return;}
  ...
  root.addEventListener('keydown',event=>{const n=event.target.closest('[data-rg-node]'),e=event.target.closest('[data-rg-edge]'),s=scope(project);if(!s)return;const v=view(s);if(e&&['Enter',' '].includes(event.key)){event.preventDefault();e.dispatchEvent(new MouseEvent('click',{bubbles:true}));return;}if(!event.key.startsWith('Arrow')||!event.target.closest('.rg-viewport'))return;event.preventDefault();const delta={ArrowLeft:[-24,0],ArrowRight:[24,0],ArrowUp:[0,-24],ArrowDown:[0,24]}[event.key];if(n&&event.altKey){const p=v.positions[n.dataset.rgNode];p.x=Math.max(0,p.x+delta[0]);p.y=Math.max(0,p.y+delta[1]);redraw({id:n.dataset.rgNode,kind:'node'});}else {v.pan.x+=delta[0];v.pan.y+=delta[1];transform();}},{signal});
  root.addEventListener('pointerdown',event=>{if(event.button!==0||!event.target.closest('.rg-viewport'))return;const s=scope(project),v=view(s);if(!v.panMode)return;const node=event.target.closest('[data-rg-node]');dragging={id:node?.dataset.rgNode,startX:event.clientX,startY:event.clientY,pan:{...v.pan},position:node?{...v.positions[node.dataset.rgNode]}:null,pointer:event.pointerId};root.setPointerCapture(event.pointerId);},{signal});
  root.addEventListener('pointermove',event=>{if(!dragging||event.pointerId!==dragging.pointer)return;const v=view(scope(project)),dx=event.clientX-dragging.startX,dy=event.clientY-dragging.startY;if(Math.abs(dx)+Math.abs(dy)>4)dragged=true;if(dragging.id){v.positions[dragging.id]={x:Math.max(0,dragging.position.x+dx/v.zoom),y:Math.max(0,dragging.position.y+dy/v.zoom)};const el=[...root.querySelectorAll('[data-rg-node]')].find(n=>n.dataset.rgNode===dragging.id);if(el){el.style.left=v.positions[dragging.id].x+'px';el.style.top=v.positions[dragging.id].y+'px';}}else {v.pan={x:dragging.pan.x+dx,y:dragging.pan.y+dy};transform();}},{signal});
  const end=()=>{if(dragging?.id){const id=dragging.id;dragging=null;redraw({id,kind:'node'});}else dragging=null;};root.addEventListener('pointerup',end,{signal});root.addEventListener('pointercancel',end,{signal});
  let lastSmall=innerWidth<=700;resizeObserver=new ResizeObserver(()=>{const small=innerWidth<=700;if(small!==lastSmall){lastSmall=small;redraw();}});resizeObserver.observe(root);
 }
 return {ingest,render,mount,disconnect:project=>{const p=projects.get(project);if(p)p.disconnected=true;},reset:()=>{disposeMount();resizeObserver?.disconnect();projects.clear();views.clear();activeProject='';}};
}
