...
export function createWorkspaceGraph({esc,diagramLinks=false,label=v=>statusLabels[v]||v,stamp=v=>v?new Date(v*1000).toLocaleString('ko-KR'):'미기록',onInteractionEnd=()=>{}}){
 ...
 function graph(snapshot,v){const small=innerWidth<=700,layout=graphLayout(snapshot.nodes,small);
  ...
  const paths=snapshot.edges.filter(e=>positions[e.from]&&positions[e.to]).map((e,index)=>{
   ...
   return `<g class="rg-edge ${planned(e)?'is-planned':''} ${selectedEdge?'is-selected':''}" data-rg-edge-id="${esc(e.id)}" data-rg-route-kind="${route.type}"><path class="rg-edge-line" d="${path}" marker-end="url(#rg-arrow)"/><path class="rg-edge-hit" d="${path}" id="rg-edge-${esc(e.id)}" data-rg-edge="${esc(e.id)}" tabindex="0" role="button" aria-label="${esc(`${snapshot.nodes.find(n=>n.id===e.from)?.name}에서 ${snapshot.nodes.find(n=>n.id===e.to)?.name} · ${relationLabels[e.kind]||e.kind} · ${planned(e)?'예정':'기록'}`)}"/></g>`;
  }).join('');
  ...
  return `<div class="rg-viewport ${v.panMode?'is-moving':''}" tabindex="0" role="group" aria-label="역할 그래프 · 방향키로 이동"><div class="rg-scene"><svg class="rg-lines" width="${width}" height="${height}" viewBox="${x} ${y} ${width} ${height}" aria-label="역할 관계"><defs><marker id="rg-arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z"/></marker></defs>${paths}${labels}</svg>${snapshot.nodes.map(node=>{const p=positions[node.id];return `<button type="button" class="rg-node ${selected?.kind==='node'&&selected.id===node.id?'is-selected':''} ${related.has(node.id)?'is-related':''} ${planned(node)?'is-planned':''}" id="rg-node-${esc(node.id)}" data-rg-node="${esc(node.id)}" aria-pressed="${selected?.kind==='node'&&selected.id===node.id}"><span class="rg-node-kind">${esc(kindLabels[node.kind]||node.kind)}${planned(node)?' · 예정':''}</span><strong>${esc(node.name)}</strong><span class="rg-node-state">${esc(status(node.status))}</span><span class="rg-node-model">${esc(summaryNode(node))}</span></button>`;}).join('')}</div></div>`;
 }
