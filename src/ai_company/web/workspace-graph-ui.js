...
function edgeGeometry(edge,positions,width,height,index=0){
 const from=positions[edge.from],to=positions[edge.to],reverse=edge.kind==='revision_return'||to.y<=from.y;
 const x1=from.x+width/2,y1=from.y+(reverse?0:height),x2=to.x+width/2,y2=to.y+(reverse?height:0);
 return {path:reverse?`M ${x1} ${y1} C ${Math.max(x1,x2)+90} ${y1-60}, ${Math.max(x1,x2)+90} ${y2+60}, ${x2} ${y2}`:`M ${x1} ${y1} C ${x1} ${(y1+y2)/2}, ${x2} ${(y1+y2)/2}, ${x2} ${y2}`,x:(x1+x2)/2+9,y:(y1+y2)/2-8+index%2*16};
}
...
export function createWorkspaceGraph({esc,label=v=>statusLabels[v]||v,stamp=v=>v?new Date(v*1000).toLocaleString('ko-KR'):'미기록'}){
 ...
 function nodeInfo(node){if(node.kind==='document')return `<p>${node.document_kind==='approval'?'후보 수용을 결정하는 승인 문서입니다.':'이 실행의 시스템 보고 문서입니다.'}</p><p>아래 실행별 링크에서 원문과 대상 식별자를 확인할 수 있습니다.</p><details><summary>문서 참조</summary><pre>${esc(JSON.stringify(node.reference||{},null,2))}</pre></details>`;const a=node.assignment||{},r=a.requested||{},o=a.observed||{};return `<dl class="rg-facts"><div><dt>종류</dt><dd>${esc(kindLabels[node.kind]||node.kind)} · ${planned(node)?'예정':'저장된 기록'}</dd></div><div><dt>현재 일</dt><dd>${esc(node.current_task_title||node.task_title||node.current_work||node.responsibility||'연결된 작업 없음')}</dd></div><div><dt>요청</dt><dd>${esc(r.model||'미배정')} · ${esc(r.reasoning_effort||'추론 미설정')}<br>Ultracode ${r.ultracode_enabled===true?'요청함':r.ultracode_enabled===false?'요청 안 함':'미확인'}</dd></div><div><dt>관측 설정</dt><dd>${o.status==='observed'?`${esc(o.model||'모델 미확인')} · ${esc(o.reasoning_effort||'추론 미확인')}`:'미확인'}</dd></div><div><dt>관측 근거</dt><dd>${esc(o.source||'없음')} · ${esc(o.scope||'미확인')}<br>모델의 자기 설명을 근거로 사용하지 않습니다.</dd></div>${node.wait_reason?`<div><dt>대기 이유</dt><dd>${esc(node.wait_reason)}${node.resume_at?`<br>재개 예약 ${esc(stamp(node.resume_at))}`:''}</dd></div>`:''}${a.quota?.status?`<div><dt>공유 한도</dt><dd>${esc(status(a.quota.status))}${a.quota.reset_at?` · ${esc(stamp(a.quota.reset_at))}`:''}</dd></div>`:''}</dl>${node.handoffs?.length?`<details><summary>담당 이관 ${node.handoffs.length}건</summary><p>역할과 작업은 유지하고 담당 세션만 바뀝니다.</p><pre>${esc(JSON.stringify(node.handoffs,null,2))}</pre></details>`:''}<details><summary>원본 참조</summary><pre>${esc(JSON.stringify(node.reference||{},null,2))}</pre></details>`;}
 ...
 function summaryNode(node){if(node.kind==='document')return node.document_kind==='approval'?'승인 원문':'시스템 보고';const a=node.assignment||{},o=a.observed||{},r=a.requested||{};return `${o.status==='observed'?'관측':'요청'} · ${shortModel(o.status==='observed'?o.model:r.model)}`;}
 ...
 function mount(container,overview,project){
  ...
  function redraw(focus){root.innerHTML=body(project);geometry();if(focus)restoreFocus(focus.id,focus.kind);}
  function geometry(){const s=scope(project),v=s&&view(s),scene=root.querySelector('.rg-scene');if(!scene||!v)return;scene.style.width=v.canvas.width+'px';scene.style.height=v.canvas.height+'px';for(const el of root.querySelectorAll('.rg-node')){const p=v.positions[el.dataset.rgNode];el.style.left=p.x+'px';el.style.top=p.y+'px';el.style.width=v.nodeWidth+'px';el.style.height=v.nodeHeight+'px';}transform();}
  ...
  root.addEventListener('pointermove',event=>{if(!dragging||event.pointerId!==dragging.pointer)return;const v=view(scope(project)),dx=event.clientX-dragging.startX,dy=event.clientY-dragging.startY;if(Math.abs(dx)+Math.abs(dy)>4)dragged=true;if(dragging.id){v.positions[dragging.id]={x:Math.max(0,dragging.position.x+dx/v.zoom),y:Math.max(0,dragging.position.y+dy/v.zoom)};const el=[...root.querySelectorAll('[data-rg-node]')].find(n=>n.dataset.rgNode===dragging.id);if(el){el.style.left=v.positions[dragging.id].x+'px';el.style.top=v.positions[dragging.id].y+'px';}const s=scope(project);for(const [index,g] of [...root.querySelectorAll('[data-rg-edge-id]')].entries()){const e=s.edges.find(e=>e.id===g.dataset.rgEdgeId),route=edgeGeometry(e,v.positions,v.nodeWidth,v.nodeHeight,index);g.querySelectorAll('path').forEach(path=>path.setAttribute('d',route.path));const text=g.querySelector('text');text.setAttribute('x',route.x);text.setAttribute('y',route.y);}}else {v.pan={x:dragging.pan.x+dx,y:dragging.pan.y+dy};transform();}},{signal});
