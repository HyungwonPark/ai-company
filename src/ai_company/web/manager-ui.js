...
export function createManagerUI({esc,badge,label,stamp,documents,planContent}) {
  ...
  function render(overview, {connected, composer, history, technical, archive, tab='conversation'}) {
    ...
    const status=candidatePending?'후보 승인 대기':run?(run.state==='pending'?'실행 준비':label(run.state)):canReview?'계획 확인 대기':request?({pending:'PM 대기',running:'계획 작성 중'}[request.state]||label(request.state)):'PM과 시작';
