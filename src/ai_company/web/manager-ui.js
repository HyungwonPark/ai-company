...
export function createManagerUI({esc,badge,label,stamp,documents,planContent}) {
  ...
  function render(overview, {connected, composer, history, technical, archive, tab='conversation'}) {
    ...
    const status=candidatePending?'후보 승인 대기':run?label(run.state):canReview?'계획 확인 대기':request?label(request.state):'PM과 시작';
    ...
    const pm=config?.agents?.find(agent=>agent.roles?.includes('pm'))||request?.requested_configuration||overview.readiness?.pm_requested;
    ...
    return `<div class="manager-home"><header class="manager-heading"><h1>매니저</h1><button class="quiet" data-action="create-project">${workspaceIcon('plus')}<span>새 프로젝트</span></button></header>
      <article class="manager-focus ${plan?'plan':''}" ${plan?`data-plan-id="${esc(plan.id)}"`:''} aria-label="현재 작업"><div class="manager-focus-label"><span class="manager-status-dot"></span>${esc(status)}</div><h2>목표</h2><p class="manager-goal">${esc(summary||(roles.length?`${roles.length}개 역할 · 완료 조건 ${displayPlan.content.completion_criteria?.length||0}개`:'만들고 싶은 것을 PM과 함께 구체화하세요.'))}</p>
      ...
      <div class="planning-workspace" data-planning-tab="${esc(tab)}"><section class="manager-compose" aria-label="PM과 대화"><header class="manager-section-heading"><h2>대화</h2><span>${esc(pmName)}</span></header><div class="discussion">${conversation||'<div class="discussion-welcome"><span class="manager-agent-icon">'+workspaceIcon('manager')+'</span><h3>어떤 것을 만들까요?</h3><p>목표를 이야기하면 PM이 필요한 역할과 계획을 제안합니다.</p></div>'}${awaiting?`<div class="discussion-status" role="status">${badge(request.state)}<span>${['waiting_quota','waiting_retry','waiting_capacity'].includes(request.state)?'요청은 저장되어 있습니다. 예약된 재개를 기다립니다.':'PM의 제안을 기다리고 있습니다.'}</span>${request.resume_at?`<time>${stamp(request.resume_at)}</time>`:''}</div>`:request?.state==='blocked'?'<div class="discussion-status"><strong>확인이 필요합니다</strong><span>자동 계획을 준비하지 못했습니다. 상세의 요청 근거를 확인하세요.</span></div>':''}</div><div class="conversation-suggestions" aria-label="대화 예시">${[['역할 추천','이 목표에 필요한 역할과 각 역할의 책임을 추천해주세요.'],['역할 추가','팀에 역할을 추가하고 싶어요. '],['더 단순하게','역할과 계획을 더 단순하게 정리해주세요.']].map(([name,text])=>`<button data-action="suggest-message" data-suggestion="${text}">${name}</button>`).join('')}</div>${composer}</section>
