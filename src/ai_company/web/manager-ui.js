...
const skillStates={recommended:'추천',reviewed:'내용 검토',assigned:'계획 배정',delivered:'실행 전달',rejected:'보류',unavailable:'사용 불가',approved_document:'내용 검토 완료',review_pending:'검토 전'};
...
export function createManagerUI({esc,badge,label,stamp,documents,planContent}) {
  ...
  function render(overview, {connected, composer, history, technical, archive, execution='', tab='conversation'}) {
    ...
    const skillSelection=displayPlan?.content?.skill_selection;
    ...
    const revisionStatus=plan?.revision_action==='automatic'?'PM 자동 수정 중':plan?.revision_action==='master_decision'?'답변 필요':plan?.revision_action==='limit_reached'?'수정 한도 도달':plan?.revision_action==='blocked'?'환경 확인 필요':'계획 수정 필요';
    const status=candidatePending?'내 확인 필요':run?(run.state==='pending'?'실행 준비':label(run.state)):needsFreshPlan?'새 계획 필요':needsSpec?'실행 범위 검토':plan?.status==='reviewing'?(plan.review_problem?'계획 검토 확인 필요':'계획 검토 중'):plan?.status==='needs_revision'?revisionStatus:canReview?'계획 확인 필요':plan?.status==='confirmed'?'실행 준비':request?({pending:'PM 접수 완료',running:'PM 답변 작성 중',completed:'PM 답변 도착',answer_needed:'답변 필요',blocked:'문제 발생',waiting_quota:'사용량 회복 대기',waiting_retry:'자동 재시도 대기',waiting_capacity:'담당 배정 대기',waiting_dependencies:'앞 작업 대기',waiting_dependency:'앞 작업 대기',waiting_role_repair:'수정 작업 대기',reconciliation_required:'상태 확인 필요',needs_context_handoff:'인수인계 확인 필요'}[requestState]||label(requestState)):'PM과 시작';
    ...
    const roleRows=roles.map((role,index)=>{
      ...
      const skillItems=Array.isArray(skillSelection?.roles?.[role.key])?skillSelection.roles[role.key]:[];
      const skillTask=(overview.tasks||[]).find(task=>task.id===current?.task_id&&task.skill_delivery?.selection_digest===skillSelection?.digest&&task.skill_delivery?.role_key===role.key);
      const delivery=skillTask?.skill_delivery;
      const skillOutcome=skillSelection?.role_outcomes?.[role.key]||skillSelection?.outcome;
      const skillFallback=skillOutcome==='lookup_failed'?`조회 실패 · ${esc(skillOutcome===skillSelection.outcome?skillSelection.reason||'원인을 확인해 주세요.':'이 역할의 자료 조회 실패')} ${skillSelection.can_continue===true?'기존 지침으로 진행할 수 있습니다.':'진행 가능 여부는 계획 검토에서 확인합니다.'}`:skillOutcome==='review_pending'?'후보를 검토 중입니다.':skillOutcome==='no_matching_document'?'조사한 저장소에서 이 역할의 SKILL.md를 찾지 못했습니다.':skillOutcome==='not_searched'?'이 역할의 공개 자료는 이번 조회 한도에서 조사하지 않았습니다.':skillOutcome==='lookup_pending'?'이 역할의 공개 조회 결과가 아직 확인되지 않았습니다.':skillOutcome==='not_allowlisted'?'이 역할에 허용된 공개 조사어가 없습니다.':skillOutcome==='no_results'?'이 역할의 공개 검색 결과가 없습니다.':skillOutcome==='search_found_unpinned'?'공개 저장소를 찾았지만 이 역할의 문서는 확인하지 못했습니다.':skillOutcome==='lookup_not_configured'?'공개 조사어가 설정되지 않았습니다.':'현재 지침으로 진행합니다.';
      const skillBrief=skillSelection?`<div class="manager-agent-skills"><strong>작업 지침</strong>${skillItems.length?skillItems.map(item=>{const delivered=Boolean(item.selected&&['process_started','executor_returned'].includes(delivery?.phase)&&delivery.documents?.some(document=>document.skill_id===item.skill_id&&document.bundle_sha256===item.bundle_sha256));return `<div class="manager-skill-row"><div><strong>${esc(item.name)}</strong><span>${esc(delivered?`실행 전달 · ${delivery.mode==='fixture'?'모의':'실행기'}`:item.selected?'계획에 배정':skillStates[item.status]||skillStates[skillSelection.status]||'검토 전')} · ${item.selected?'선택됨':'미선택'}</span></div><p>${esc(item.reason||'추천 근거 확인 중')}</p><details data-persist-key="team-skill:${esc(displayPlan.id)}:${esc(role.key)}:${esc(item.name)}"><summary>${esc(item.name)} 상세</summary><dl class="detail-grid"><div><dt>출처</dt><dd class="mono">${esc(item.source_url||'확인 중')}</dd></div><div><dt>버전</dt><dd>${esc(item.version||'확인 중')}</dd></div><div><dt>파일 묶음 해시</dt><dd class="mono">${esc(item.bundle_sha256||'확인 중')}</dd></div><div><dt>관련 요구사항</dt><dd>${esc(Array.isArray(item.requirements)?item.requirements.join(' · '):'연결 없음')}</dd></div><div><dt>도구·의존성</dt><dd>${esc(Array.isArray(item.dependencies)?item.dependencies.join(' · '):'추가 없음')}</dd></div><div><dt>필요 권한</dt><dd>${esc(Array.isArray(item.permissions)?item.permissions.join(' · '):'추가 없음')}</dd></div></dl>${delivered?`<p>전달 기록 · ${esc(delivery.delivery_id)} · 효과 미확인</p>`:''}</details></div>`}).join(''):`<p>${skillFallback}</p>`}</div>`:'';
      return `<li class="manager-agent"><div class="manager-agent-top"><span class="manager-agent-icon">${workspaceIcon(/test/.test(role.key)?'tests':'developer')}</span><strong>${esc(roleName)}</strong>${badge(roleStatus)}</div><p>${esc(responsibility||'상세 계획에서 책임 범위를 확인하세요.')}</p><div class="manager-agent-bottom"><span>${esc(model)}</span>${!run?`<button class="quiet" data-action="suggest-message" data-suggestion="${esc(roleName)} 역할의 책임과 완료 조건을 함께 조정하고 싶어요." aria-label="${esc(roleName)} 역할 조정">조정 ${workspaceIcon('arrow')}</button>`:''}</div>${skillBrief}${skillSelection?.role_reasons?.[role.key]?`<p class="small muted">${esc(skillSelection.role_reasons[role.key])}</p>`:''}${node?.wait_reason?`<p class="small muted">${esc(node.wait_reason)}</p>`:''}</li>`;
    }).join('');
    const requestProblem=['blocked','failed','stopped','reconciliation_required','needs_reconciliation','needs_context_handoff','no_eligible_agent','superseded'].includes(requestState);
    const openQuestions=(request?.requirements_feedback?.questions||[]).filter(item=>item.status==='open');
    const runProblem=run&&['blocked','failed','stopped','reconciliation_required','needs_reconciliation','needs_context_handoff','no_eligible_agent','superseded','rejected'].includes(String(run.state).toLowerCase());
    const waiting=run?.wait_reason||run?.reason||plan?.review_problem||requestDisplay.reason;
    const resumeAt=run?.resume_at??requestDisplay.resumeAt;
    const revisionNext=plan?.revision_action==='automatic'?'PM이 기술 지적을 자동으로 보완하고 다시 검수받고 있습니다.':plan?.revision_action==='master_decision'?'중요한 결정이 필요합니다. 검수 지적을 확인하고 답해 주세요.':plan?.revision_action==='limit_reached'?'자동 보완 2회 한도에 도달했습니다. 지적을 확인하고 다음 범위를 결정해 주세요.':plan?.revision_action==='blocked'?`자동 보완이 멈췄습니다. ${plan.revision_problem||'원인을 확인해 주세요.'}`:'검수 지적을 확인하고 PM에게 계획 수정을 요청하세요.';
    const nextText=candidatePending?'이 실행의 결과 승인 요청을 확인하세요. 수용 여부는 별도로 결정합니다.':runProblem?'실행에 문제가 생겼습니다. 원인과 재개 조건을 확인하세요.':needsFreshPlan?freshPlanReason:needsSpec?'실행 명세를 먼저 저장한 뒤 새 계획을 받아주세요. 아직 개발을 시작하지 않습니다.':plan?.status==='reviewing'?(plan.review_problem?'계획 검토가 멈췄습니다. 원인을 확인한 뒤 재개 조건을 검토하세요.':'별도 계획 검토가 끝나면 정확한 계획을 확인할 수 있습니다.'):plan?.status==='needs_revision'?revisionNext:canReview?'확정 전에는 이 계획의 작업을 시작하지 않습니다.':run?'확정한 계획의 실제 작업과 검수 결과를 확인하세요.':plan?.status==='confirmed'?'계획 확정은 저장됐습니다. 조정기가 실행을 준비합니다.':requestState==='answer_needed'?'PM의 필수 질문을 확인하고 중요한 선택에 답해 주세요.':requestProblem?'PM이 계획을 준비하지 못했습니다. 요청 근거를 확인하고 대화에서 범위를 조정하세요.':request&&!['completed','stale'].includes(request.state)?'PM 요청은 저장됐습니다. 답변이 도착하면 팀과 계획을 확인하세요.':!plan&&displayPlan?'팀을 조정하는 중입니다. 최신 PM 답변이 도착한 뒤 새 계획을 확정할 수 있습니다.':'목표와 원하는 역할을 PM에게 이야기하세요.';
    ...
    const action=candidatePending?`<a class="manager-primary-link" href="${href('approvals')}&approval=${encodeURIComponent(run.approval_id)}">승인 요청 확인 ${workspaceIcon('arrow')}</a>`:needsFreshPlan?execution?`<button class="primary" data-action="focus-execution">실행 범위 확인 ${workspaceIcon('arrow')}</button>`:`<a class="manager-primary-link" href="${href('project')}">새 계획 요청 ${workspaceIcon('arrow')}</a>`:needsSpec?execution?`<button class="primary" data-action="focus-execution">실행 범위 확인 ${workspaceIcon('arrow')}</button>`:`<a class="manager-primary-link" href="${href('project')}">실행 범위 확인 ${workspaceIcon('arrow')}</a>`:canReview?`<button class="primary" data-action="review-plan" data-id="${esc(plan.id)}" ${!connected||!plan.digest||project.source==='fixture'?'disabled':''}>계획 검토·확정 ${workspaceIcon('arrow')}</button>`:run?`<a class="manager-primary-link" href="${href('progress')}">작업 진행 보기 ${workspaceIcon('arrow')}</a>`:`<button data-action="focus-message">${received?'PM과 대화':'대화 시작'} ${workspaceIcon('arrow')}</button>`;
    ...
    const awaiting=request&&!['completed','stale','answer_needed'].includes(requestState)&&!requestProblem;
    ...
    const connection=`<details class="manager-connections" data-persist-key="connections"><summary>연결 <span>${live(workers.automation)?'PM 가동':'PM 상태 확인'} · ${live(workers.translation)?'번역 가동':'번역 상태 확인'}</span></summary><dl><div><dt>PM</dt><dd>${esc(pmName)} <small>요청 설정 · 실제 적용 근거는 상세에서 확인</small></dd></div><div><dt>번역</dt><dd>${esc(modelLabel(workers.translation?.configuration?.agents?.[0]?.model||overview.translation_summary?.requested_configuration?.model))}<small>개발에 사용하려면 개발 역할의 실행 검증이 필요합니다.</small></dd></div><div><dt>개발</dt><dd>${config?esc([...new Set((config.agents||[]).filter(agent=>agent.roles?.includes('developer')).map(agent=>modelLabel(agent.model,agent.reasoning_effort)))].join(' / ')||'등록된 후보 없음'):'설정 정보 대기'}<small>등록된 요청 후보 · 실행 가능 여부와 실제 배정은 별도 확인</small></dd></div></dl>${pilot?'<p>현재 실행 범위는 역할 상태 집계의 개발·검사 두 파일입니다. 다른 목표는 실행 범위를 먼저 준비해야 합니다.</p>':scope.length?`<details><summary>허용 범위</summary><ul>${scope.map(path=>`<li class="mono">${esc(path)}</li>`).join('')}</ul></details>`:'<p>실행 허용 범위는 아직 이 화면에 연결되지 않았습니다. 계획 확정 전에 상세에서 확인하세요.</p>'}</details>`;
    return `<div class="manager-home"><header class="manager-heading"><h1>계획</h1><button class="quiet" data-action="create-project">${workspaceIcon('plus')}<span>새 프로젝트</span></button></header>
      ...
      ${execution?`<section class="manager-execution" aria-label="계획의 실행 준비">${execution}</section>`:''}
