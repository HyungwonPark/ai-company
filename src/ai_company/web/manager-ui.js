...
export function createManagerUI({esc,badge,label,stamp,documents,planContent}) {
  ...
  function render(overview, {connected, composer, history, technical, archive}) {
    ...
    return `<div class="manager-home"><header class="manager-heading"><div><h1>매니저</h1></div><button class="quiet" data-action="focus-message" aria-label="PM에게 새 목표 전달">${workspaceIcon('plus')}<span>목표 전달</span></button></header>
      <article class="manager-focus ${plan?'plan':''}" ${plan?`data-plan-id="${esc(plan.id)}"`:''} aria-label="현재 작업"><div class="manager-focus-label"><span class="manager-status-dot"></span>${esc(status)}</div><h2>목표</h2><p class="manager-goal">${esc(summary||(roles.length?`${roles.length}개 역할 · 완료 조건 ${plan.content.completion_criteria?.length||0}개`:'목표를 남기면 계획부터 확인할 수 있습니다.'))}</p>
      ...
      ${plan?`<details class="manager-plan-detail" data-persist-key="manager-plan:${esc(plan.id)}"><summary>계획</summary>${planContent(plan)}${documents.meta(`plan:${plan.id}`)}</details>`:`<details class="manager-plan-detail" data-persist-key="manager-goal"><summary>원문</summary><div class="message-content">${esc(documents.text(`project:${project.id}`,'goal',project.goal))}</div>${documents.meta(`project:${project.id}`)}</details>`}</article>
      <div class="manager-columns"><section class="manager-team" aria-label="계획의 역할 배치"><header class="manager-section-heading"><h2>에이전트 <span>${roles.length}</span></h2><a href="${href('progress')}">전체 보기 ${workspaceIcon('arrow')}</a></header>${roles.length?`<div class="manager-parallel">${workspaceIcon('progress')}<span>${independent>1?`${independent}개 역할은 서로 기다릴 필요 없이 시작할 수 있습니다.`:'계획에 지정된 순서로 진행합니다.'}</span></div><ul class="manager-agents">${roleRows}</ul>`:'<p class="manager-empty">PM이 계획을 제안하면 담당 역할이 여기에 표시됩니다.</p>'}</section>
      <section class="manager-inbox" aria-label="확인할 항목"><h2>알림</h2><a href="${href('approvals')}"><span class="manager-inbox-icon">${workspaceIcon('approvals')}</span><span><strong>승인</strong><small>${pending.length?'검토를 기다리는 요청이 있습니다':'기다리는 요청이 없습니다'}</small></span><b>${pending.length}</b>${workspaceIcon('arrow')}</a><a href="${href('reports')}"><span class="manager-inbox-icon">${workspaceIcon('project')}</span><span><strong>보고서</strong><small>검사와 검수 결과</small></span><b>${reports.length}</b>${workspaceIcon('arrow')}</a><p>계획 확정과 후보 승인은 각각 검토합니다.</p></section></div>
      <section class="manager-compose" aria-label="PM에게 전달"><header class="manager-section-heading"><h2>대화</h2><span>Astra Ultra PM</span></header>${composer}</section>
