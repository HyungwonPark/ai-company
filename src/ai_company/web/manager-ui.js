...
export function createManagerUI({esc,badge,label,stamp,documents,planContent}) {
  ...
  function render(overview, {connected, composer, history, technical, archive, execution='', tab='conversation'}) {
    ...
    const roleRows=roles.map((role,index)=>{
      ...
      const skillFallback=skillSelection?.outcome==='lookup_failed'?`조회 실패 · ${esc(skillSelection.reason||'원인을 확인해 주세요.')} ${skillSelection.can_continue===true?'기존 지침으로 진행할 수 있습니다.':'진행 가능 여부는 계획 검토에서 확인합니다.'}`:skillSelection?.outcome==='review_pending'?'후보를 검토 중입니다.':['lookup_pending','search_found_unpinned','lookup_not_configured','no_results'].includes(skillSelection?.outcome)?esc(skillSelection.reason||'조사 상태를 확인해 주세요.'):'현재 지침으로 진행합니다.';
