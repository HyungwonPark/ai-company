...
export function createManagerUI({esc,badge,label,stamp,documents,planContent}) {
  ...
  function render(overview, {connected, composer, history, technical, archive}) {
    ...
    return `<div class="manager-home"><header class="manager-heading"><div><h1>매니저</h1></div><button class="quiet" data-action="focus-message" aria-label="PM에게 새 목표 전달">${workspaceIcon('plus')}<span>목표 전달</span></button></header>
      ...
      ${plan?`<details class="manager-plan-detail" data-persist-key="manager-plan:${esc(plan.id)}"><summary>계획</summary>${planContent(plan)}</details>`:`<details class="manager-plan-detail" data-persist-key="manager-goal"><summary>원문</summary><div class="message-content">${esc(documents.text(`project:${project.id}`,'goal',project.goal))}</div>${documents.meta(`project:${project.id}`)}</details>`}</article>
