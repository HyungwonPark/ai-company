    ...
    if role == "developer" and contribution:
        ...
        if role == "pm" and planning:
            work += (
                ...
                "Resolve every pending_material_questions entry from the saved context. If earlier feedbacks reuse "
                "a question ID, give each resolved plan question a unique id plus the exact source_request_id "
                "and source_question_id from that feedback, preserving its exact prompt. For a repair question "
                "also copy source_plan_id and source_repair_attempt from the saved context. Never drop an older "
                "unresolved question. If the master already decided something in the original goal without a "
                "prior PM question, mark evidence_kind=master_goal, cite that exact user message, and copy its "
                "verbatim decision into both goal_quote and resolution; do not "
                "invent a question source or treat an earlier message as an answer to a later question. "
                "If pending_material_questions_omitted is nonzero, return BLOCK and explain that the saved "
                "question history needs a scoped review. "
