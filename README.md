...
현재는 외부 모델 호출 없이 실행하는 첫 기반 구현입니다. 공통 하네스에 가짜 개발자·검수자를 연결하여 반려 후 수정, 한도 중단, 중복 접수와 재시작 복구를 검증합니다. 실제 Claude Code·Codex 실행과 GitHub 작업 자동화는 다음 단계입니다.
...
작업 공간 분리는 OS 권한 격리가 아닙니다. 현재 하네스는 시뮬레이션만 허용합니다. 실제 CLI·하위 프로세스 종료, 서버 계정·자원 격리, 인증·사용량, 원격 Git·필수 CI 검증, systemd 운영은 서버 연결 후 구현·검증할 항목입니다.
