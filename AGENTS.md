...
- 초기 Python·LangGraph·SQLite 개발 루프는 단일 서버 시뮬레이션으로 검증했다. 후속 실제 실행과 초기 모의 실행의 근거를 구분한다.
- 별도의 SQLite 세션 큐와 Claude Code·Codex 프로세스 실행기, 사용자 systemd timer가 있다. 세션 완료는 작업 검사·검수나 병합 준비를 의미하지 않는다. 실제 모델 호출·제한된 격리 작업·검수와 별도 시험 큐의 Claude 이관·동일 세션 재개 기록은 후속 문서에 있다. 최신 단계별 검증과 운영 적용 여부는 `docs/live-use-readiness-2026-09-16.md`, `docs/claude-activation-2026-09-16.md`를 확인한다. c1af888 기록 기준 새 Claude 운영 패키지 적용과 마스터의 APK 입력부터 동일 실행 전체 대조는 미완료다.
...
- 사용자 화면·안내·협업 시각화를 바꿀 때는 `docs/product-experience/README.md`에서 연결한 UX·디자인·검수 지침을 읽는다.
- 이 문서는 후속 설계 기준이다. 운영 적용 상태와 실행 권한은 기존 기록·정책·명시적 승인에서 확인한다.
- 사용자가 프로젝트를 찾고 다음 행동을 이해하는지, 화면의 상태가 실제 이벤트와 일치하는지 검증한다. 기능 검사 통과만으로 사용자 검증 완료를 선언하지 않는다.
- 개발 기록은 문서의 검증 근거 형식으로 남기며 기존 계획·승인·원문·사용량 이력을 덮어쓰지 않는다.
