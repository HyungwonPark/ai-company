...
(async()=>{
 ...
 const until=async(predicate,label)=>{let state;for(let attempt=0;attempt<14;attempt++){state=await command('step');if(predicate(state))return state;}throw new Error('Fixture worker did not reach '+label+': '+JSON.stringify(state));};
