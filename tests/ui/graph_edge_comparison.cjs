...
(async()=>{
 ...
 try{for(const theme of ['light','black'])for(const width of [320,390,1440])for(const [version,file]of [['before',before],['after',after]]){
  ...
  await page.screenshot({path:path.join(out,`workspace-edges-${version}-${theme}-${width}.png`),fullPage:true});
  await page.locator('[data-rg-action=fit]').click();
  await page.screenshot({path:path.join(out,`workspace-edges-${version}-fit-${theme}-${width}.png`),fullPage:true});
  const ids=await page.locator('.rg-edge-hit').evaluateAll(xs=>xs.map(x=>x.dataset.rgEdge).sort());
  records.push({version,theme,width,ids});await context.close();
 }
