...
(async () => {
  ...
  const openAccountMenu=async()=>{
    const menu=page.locator('.workspace-account-menu');
    if(!await menu.evaluate(element=>element.open))await page.getByLabel('계정 메뉴',{exact:true}).click();
  };
  const failures = [];
  ...
  try {
    ...
    await require('./execution_specs.cjs')({page,fixtureId,output,setHTTPFixture:value=>{expectedHTTPFailure=value;}});
    ...
    await page.getByLabel('목표',{exact:true}).fill('격리된 UI 회귀 검증: 대화와 하네스 보존을 확인한다.');
    assert.equal(await page.getByLabel('초기 역할 (선택)').count(),0,'a new project does not require a technical team definition');
    await page.getByRole('button',{name:/PM과 시작/}).click();
    await page.getByRole('heading',{name:'매니저',exact:true}).waitFor();
    const secondId = await page.locator('#project-select').inputValue();
    assert.notEqual(secondId, fixtureId);
    const started=await page.evaluate(async id=>(await fetch(`/api/projects/${id}/overview`)).json(),secondId);
    assert.equal(started.pm_requests.length,1,'goal queues exactly one PM request');
    ...
    assert.equal(await page.locator('.manager-stages .has-record').count(),3,'goal, team and execution are recorded after confirmation');
    ...
    assert.equal(await page.getByRole('button',{name:'보내기'}).isDisabled(),true,'offline writes disabled');
