...
(async()=>{
 ...
 try{
  ...
  currentCase='old execution and history';await page.setViewportSize({width:390,height:844});await page.locator('#rg-snapshot').selectOption(old.id);await page.waitForFunction(id=>new URLSearchParams(location.hash.split('?')[1]).get('run')===id,old.run_id);await scoped(old.run_id,'snapshot selection');
  ...
  await page.route('**'+endpoint,handler=>handler.fulfill({status:401,contentType:'application/json',json:{error:{code:'unauthorized'}}}));await page.getByRole('button',{name:/새로고침/,exact:false}).first().click();await page.getByRole('button',{name:'로그인',exact:true}).waitFor();assert.equal(await page.locator('.rg-node,.approval-reading,.report-list').count(),0);checks.push('offline reads retain last selected execution; injected expired login removes private views');
