...
(async()=>{
 ...
 try{
  ...
  const beforeSkip=page.url();await page.locator('.skip-link').focus();await page.keyboard.press('Enter');assert.equal(await page.evaluate(()=>document.activeElement.id),'main','skip link still focuses reading start');assert.equal(page.url(),beforeSkip,'skip link does not change selected project/run');
