...
module.exports=async function({page,context,fixtureId,output,setNetworkFixture,setHTTPFixture}){
  ...
  try{
    ...
    await page.locator('button[data-collaboration-transfer="past-repair"]').focus();
    await page.keyboard.press('Enter');
    await page.getByRole('complementary',{name:'선택한 협업 상세'}).getByRole('heading',{name:'수정 반환',exact:true}).waitFor();
    assert.equal(await page.locator('button[data-collaboration-transfer="past-repair"]').getAttribute('aria-pressed'),'true');
    assert.equal(await page.evaluate(()=>Boolean(window.transferInjected)),false);
    cursor=10001;extra=[{id:'new-request',cursor}];
    await refresh();
    await page.locator('button[data-collaboration-transfer="new-request"]').waitFor();
    ...
    await refresh();
    await page.locator('button[data-collaboration-transfer="late-fact"]').waitFor();
    ...
    assert.equal(await page.locator('button[data-collaboration-transfer="past-repair"]').getAttribute('aria-pressed'),'true','selection survives polling');
    await page.locator('.collab-node[data-node-id]').filter({has:page.locator('.collab-kind',{hasText:'프로젝트 역할'})}).first().getByRole('button').click();
    ...
    assert.equal(await page.getByText('old-request',{exact:true}).count(),0,'older snapshot cannot revert owner');
    ...
    await page.locator('button[data-collaboration-transfer="during-disconnect"]').waitFor();
    ...
    await page.locator('button[data-collaboration-transfer="reduced-motion-event"]').waitFor();
