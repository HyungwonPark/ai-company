...
(async()=>{
 ...
 try{
  ...
  for(const width of [1440,390,320])for(const theme of ['light','black'])for(const concept of ['a','b','c']){
   ...
   await page.locator(`[data-concept="${concept}"]`).click();
  ...
  await page.locator('#lab-confirm-form button[type=submit]').dblclick();
  ...
  await reset();await page.locator('[data-concept=b]').click();await click('artifact');
  ...
  await page.locator('[data-concept=c]').click();await click('review-decision');
