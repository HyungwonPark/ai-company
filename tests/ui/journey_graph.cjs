...
(async()=>{
 ...
 try{
  for(const width of [320,390,1440])for(const theme of ['light','black']){
   ...
   const old=overview.workspace_graph.snapshots.find(s=>s.run_id===fixtures.run_ids[0]);await page.locator('#journey-run').selectOption(old.id);await helpOpen('different execution');await page.locator('#journey-run').selectOption(snapshot.id);await helpOpen('execution return');
