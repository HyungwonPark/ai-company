...
(async()=>{await fs.mkdir(out,{recursive:true});const browser=await chromium.launch({headless:true,chromiumSandbox:true,...(process.env.CHROME_CHANNEL?{channel:process.env.CHROME_CHANNEL}:{})});const context=await browser.newContext({viewport:{width:390,height:844},serviceWorkers:'block'});const page=await context.newPage();page.setDefaultTimeout(6000);const errors=[];page.on('pageerror',e=>errors.push(e.message));
 ...
 try{for(const theme of ['light','black'])for(const view of ['projects','manager','progress','approvals']){await page.goto(origin+'/?workspace=legacy#'+view+(view==='projects'?'':'?project=fixture-pilot'));await page.locator('.nav').waitFor();await page.getByRole('heading',{name:view==='projects'?'프로젝트':view==='manager'?'매니저':view==='progress'?'진행':'승인',exact:true}).first().waitFor();await selectTheme(page,theme);await settledScreenshot(page,{path:path.join(out,`workspace-baseline-${view}-${theme}-390.png`),fullPage:true});}assert.deepEqual(errors,[]);console.log('Same-data baseline: existing projects/manager/progress/approvals, Light/Black 390px, zero writes PASS');}finally{await browser.close();}
