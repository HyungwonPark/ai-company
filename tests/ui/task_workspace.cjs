...
(async()=>{
 ...
 try{
  ...
  if(await page.locator('html').getAttribute('data-theme')!=='light')await page.locator('#theme').click();await stage('result');await page.getByText('검수는 끝났고,',{exact:false}).waitFor();await snap('journey-light-390-result');await stage('approval');await click('decision');assert.match(await page.locator('#detail').innerText(),/example-approval-01/);await page.locator('#decision-reviewed').check();await click('decision-submit');assert.equal((await stored()).decision.run,'example-run-01');await page.getByText('원래 후보는 pending입니다.',{exact:false}).waitFor();
  await page.locator('#run').selectOption('current');await page.getByRole('heading',{name:'요청이 없어요'}).waitFor();assert.equal(await page.locator('[data-action=decision]').count(),0);
  ...
  await stage('projects');await click('new');await page.locator('#name').fill('읽기 쉬운 로그인');await page.locator('#goal').fill('오류가 나면 다음에 누를 곳을 알고 싶어요.');await page.locator('#new-form button').click();assert.equal((await stored()).confirmed,false);await page.locator('#pm-reply').fill('안내를 짧고 쉽게 보여 주세요.');await page.locator('#pm-form button').click();await click('save-spec');assert.equal(await page.evaluate(()=>document.activeElement.dataset.action),'request-plan');assert.equal((await stored()).confirmed,false);await page.reload();assert.equal(await page.locator('#pm-reply').inputValue(),'안내를 짧고 쉽게 보여 주세요.');await click('request-plan');assert.equal(await page.evaluate(()=>document.activeElement.dataset.action),'confirm');assert.equal((await stored()).confirmed,false);await click('confirm');await page.locator('#reviewed').check();await click('confirm-submit');assert.equal((await stored()).confirmed,true);await page.reload();await page.getByRole('heading',{name:'시작 대기'}).waitFor();assert.match(await page.locator('#main').innerText(),/실제 모델·개발·검사는 실행하지 않습니다/);await snap('new-project-confirmed');
  checks.push('Natural-language input → PM fixture → spec save (0) → fresh plan (0) → explicit sample confirm (1); reload preserves draft and one sample run');
  await stage('projects');await click('new');assert.equal((await stored()).confirmed,true);assert.equal((await stored()).created.name,'읽기 쉬운 로그인');await stage('projects');await page.locator('[data-project=sample]').click();if(await page.locator('html').getAttribute('data-theme')!=='black')await page.locator('#theme').click();
