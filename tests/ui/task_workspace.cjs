...
(async()=>{
 ...
 try{
  ...
  if(await page.locator('html').getAttribute('data-theme')!=='light')await page.locator('#theme').click();await stage('result');await page.getByText('검수는 끝났고,',{exact:false}).waitFor();await snap('journey-light-390-result');await stage('approval');await click('decision');await page.locator('#detail .binding summary').click();assert.match(await page.locator('#detail').innerText(),/example-approval-01/);await page.locator('#detail .binding summary').click();await page.locator('#decision-choice').selectOption('approve');await page.locator('#decision-reviewed').check();await click('decision-submit');assert.equal((await stored()).decision.run,'example-run-01');await page.locator('#main').getByText('원래 후보 pending 유지',{exact:false}).waitFor();
