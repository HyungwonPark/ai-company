...
(async()=>{
 ...
 const backToGraph=async snapshot=>{await page.goBack();await page.locator('[data-rg-root]').waitFor();assert.equal(await page.locator('#rg-snapshot').inputValue(),snapshot.id);assert.equal(new URL(page.url()).hash,'#progress?project='+fixtures.project_id);};
 ...
 try{
  for(const width of [320,390,1440])for(const theme of ['light','black']){
   ...
   currentCase.stage='same-run report and browser back';const reportLink=page.locator('.rg-reference-links a[href^="#reports"]');await reportLink.click();await page.locator('.integrated-scope').waitFor();assert.equal(new URLSearchParams(new URL(page.url()).hash.split('?')[1]).get('run'),current.run_id);assert.ok((await page.locator('.integrated-scope').textContent()).includes(current.run_id));
   const reportIds=new Set(current.report_refs.map(r=>r.id));assert.deepEqual((await page.locator('.report-list .document-title').allTextContents()).sort(),before.reports.filter(r=>reportIds.has(r.id)).map(r=>r.title).sort());await screenshot(`${theme}-${width}-reports`);await backToGraph(current);
   currentCase.stage='same-run approval and cancelled review';await clear();await graphSelection(old);await page.locator('.rg-reference-links a[href^="#approvals"]').click();await page.locator('.approval-reading').waitFor();assert.equal(new URLSearchParams(new URL(page.url()).hash.split('?')[1]).get('run'),old.run_id);
   const approval=before.approvals.find(a=>a.id===old.approval_refs[0].id);assert.ok(approval);const review=page.locator(`.approval-reading [data-decision="approve"][data-id="${approval.id}"]`);await review.waitFor();await screenshot(`${theme}-${width}-approval`);await review.click();await page.locator('#decision-form').waitFor();assert.equal(await page.locator('#decision-form').getAttribute('data-id'),approval.id);assert.equal(await page.locator('#decision-form').getAttribute('data-digest'),approval.subject_digest);await page.locator('#dialog [data-action="close-dialog"]').click();await backToGraph(old);
   ...
   checks.push({width,theme,entry:'/',project_id:fixtures.project_id,report_run:current.run_id,approval_run:old.run_id,approval_id:approval.id,approval_status:approval.status,graph_edges:current.edges.length,back:'real page.goBack twice',relaunch:'closed browser context, restored browser cookies/localStorage, / with no query; not Android device',writes:'login/password only'});await context.close();context=null;
  }
