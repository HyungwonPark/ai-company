...
최신 운영은 **웹·PM/조정기 `23ac8a7`과 읽기 전용 pilot 카탈로그 적용 완료**다. [실제 예산·Astra 적격성·운영 적용·APK 조작 안내](project-execution-activation-2026-09-16.md)를 따른다. 번역은 기존 `5594744`와 기록을 유지했다. 아래 이전 적용·미적용 상태는 당시 기록으로 보존하며, 마스터의 새 APK 목표 입력·계획 확정과 서명키 외부 복원은 계속 미완료다.

직전 공개 웹은 별도로 승인받은 **`28404b2` 적용 완료**였으며 [현재 화면·APK 조작·복구 결과](pr12-web-activation-2026-09-16.md)를 따른다. 당시 두 worker와 Claude 설정의 기준은 [승인된 `5594744` 적용 결과](production-activation-2026-09-16.md)다. 웹·두 worker·Claude 개발 대체 후보를 한 번 교체했고, 제한된 번역 복구는 4건 반영·2건 보호검사 차단으로 종료했다. 남은 2건의 의미 재검사 결과는 아래에 구분한다. 이전 적용 대기 기록은 각 시점의 기록으로 보존한다.

후속 구축 단계인 [프로젝트별 실행 명세와 PM 제안·계획 확정](project-execution-specs-2026-09-16.md)은 **코드 `23ac8a7`·PR #13의 개발·격리 검증·독립 검수·원격 CI·후속 패키지 준비 완료**로 기록했다. 당시 미적용이었으며 이후 위 한정 승인에 따라 웹·PM/조정기·pilot 카탈로그만 운영에 적용했다. 기존 공개 적용과 실행 범위는 보존하며, APK에서 마스터가 직접 조작하는 검증과 서명키의 외부 보관·복원 확인은 계속 미완료다. [저장 번역 재검사](translation-saved-recheck-2026-09-16.md)는 복사본에서 1건만 통과했고 실제 오역 1건은 실패를 유지했다. 운영의 두 실패를 덮어쓰지 않았다.
...
| 항목 | 완료·차단과 다음 행동 |
|---|---|
| 공개 화면 | `5594744` → `28404b2`의 승인된 적용 기록 보존. 최신 `23ac8a7` 웹·PM/조정기·pilot 카탈로그 적용 완료. 번역 worker는 `5594744` 유지 |
| 상시 실행 | 자동화·번역 두 서비스 enabled/active. 중복 거부·중단 후 복구 검증 완료, 실제 호스트 재부팅은 미실시 |
| 번역 | 원래 실패 6건 보존. 기존 복구 산출물 4건 완료·2건 보호검사 차단, 당시 추가 호출 4회·재예약 없음. 이번 저장 결과 대조는 추가 호출 0이며 복사본 1건 통과·1건 실제 오역 거부, 운영 미적용 |
| 프로젝트 실행 명세 | PR #13 개발 단계 완료. 별도 승인한 `23ac8a7`의 기존 pilot 범위만 운영 적용 완료. 마스터가 직접 확정한 새 실행의 실제 대조는 대기 |
