...
- 원본 키는 서버 사용자 전용 `~/.local/share/ai-company/android-signing`에 있고 동일 서버 백업만 있다. Git·공개 릴리스·로그에 키나 비밀번호를 올리지 않았다.
- **서버 외부 백업과 복구 실습은 미완료**다. 보관 위치 선택을 요청했다. 지정된 사적 경로로 암호화본을 전달하고, 별도 안전한 기기에서 복호화·keystore 열기·공개 인증서 지문 일치·암호화본 SHA-256 기록을 확인해야 완료다. 암호는 백업 파일과 별도 경로로 관리한다. 같은 서버의 복사본을 외부 백업으로 세지 않는다.
