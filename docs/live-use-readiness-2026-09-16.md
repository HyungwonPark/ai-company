...
최신 공개 웹은 별도로 승인받은 **`28404b2` 적용 완료**이며 [현재 화면·APK 조작·복구 결과](pr12-web-activation-2026-09-16.md)를 따른다. 두 worker와 Claude 설정의 기준은 [승인된 `5594744` 적용 결과](production-activation-2026-09-16.md)다. 웹·두 worker·Claude 개발 대체 후보를 한 번 교체했고, 제한된 번역 복구는 4건 반영·2건 보호검사 차단으로 종료했다. 남은 2건의 의미 재검사 결과는 아래에 구분한다. 이전 적용 대기 기록은 각 시점의 기록으로 보존한다.

다음 구축 단계인 [프로젝트별 실행 명세와 PM 제안·계획 확정](project-execution-specs-2026-09-16.md)은 **코드 `9e6c0c1`·PR #13의 개발·격리 검증·독립 검수·원격 CI·후속 패키지 준비를 완료했고 운영 미적용**이다. 기존 공개 적용과 실행 범위는 보존하며, APK에서 마스터가 직접 조작하는 검증과 서명키의 외부 보관·복원 확인은 계속 미완료다. [저장 번역 재검사](translation-saved-recheck-2026-09-16.md)는 복사본에서 1건만 통과했고 실제 오역 1건은 실패를 유지했다. 운영의 두 실패를 덮어쓰지 않았다.
...
| 프로젝트 실행 명세 | PR #13 개발 단계 완료·운영 미적용. 명세 버전·확정 경계·프로젝트 격리·누적 예산과 후보 이미지/후속 적용·복구안 준비 완료 |
