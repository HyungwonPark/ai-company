...
2. 실제 저장소를 수정하는 Claude 역할에 대해 허용 경로·비허용 경로·도구 샌드박스와 Workflow 자식의 제한을 검증해야 한다. 후속 [내장 Bash 재현](claude-native-sandbox-2026-09-16.md)에서는 `socat` 의존성을 검증 경로에서 해결한 뒤에도 `apply-seccomp`의 중첩 격리 생성이 실패했다. 이번 도구 없는 데이터 판정을 쓰기 격리 증거로 대체하지 않는다.
