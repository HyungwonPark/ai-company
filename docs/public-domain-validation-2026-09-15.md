...
이 문서의 최초 연결 증거는 보존한다. 이후 아이디·비밀번호 전환과 현재 실행본은
[비밀번호 로그인 적용 기록](password-login-validation-2026-09-15.md)을 따른다.
...
아이디는 `edward`다. 개인적으로 발급받은 임시 비밀번호로 로그인한 뒤,
현재 비밀번호와 새 비밀번호(10~128자)를 입력해 변경한다. 변경 전에는 프로젝트 조회와
계획 확정을 포함한 관리 기능을 사용할 수 없다. 기존 접근 토큰은 공개 서버에서 받지 않는다.
