...
return `<h2>진행</h2><div class="run-meta"><span>${oldRun()?'1차 · 검수 완료, 후보 결정 대기':'2차 · 개발과 검사'}</span></div><nav class="tabs" aria-label="진행 보기">${[['work','업무'],['team','팀'],['history','기록']].map(([id,label])=>`<button data-tab="${id}" aria-pressed="${state.tab===id}">${label}</button>`).join('')}</nav>${state.tab==='team'?team():state.tab==='history'?history():work()}${binding()}`;}
