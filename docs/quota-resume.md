...
후속 요구사항은 [역할별 에이전트 자동 이관](agent-failover.md)에 정리했다. 적격 대체자가 있으면 먼저 이관하고, 대체자가 없을 때 아래 예약 대기 기능을 사용하도록 확장한다. 이 문서의 아래 내용은 현재 구현된 동일 세션 재개 동작을 설명하며, 제공자 간 자동 이관과 최종 검수 통합의 구현 완료를 뜻하지 않는다.
